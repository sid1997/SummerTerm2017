...
clf = SVC(C=0.51,gamma=0.001,class_weight='balanced')#applying SVC model with respective parameters
clf.fit(X_train,Y1_train)#fitting model on training set 
y1_pred = clf.predict(X_test)#making predictions on test set and storing it in y1_pred
#print y1_pred
...
clf2 = SVC(kernel='sigmoid',C=10,gamma=0.00491,class_weight='balanced')
...
clf3 = SVC(C=1.1,gamma=0.00058,class_weight='balanced')
...
clf4 = SVC(C=0.171,gamma=0.0033,class_weight='balanced')
...
for i in range(0,32):
    plt.scatter(X.iloc[:,i],Y2)
    plt.xlabel('attribute '+str(i+1))
    plt.ylabel('Schiller')
    plt.title('Cervical Cancer')
    plt.show()
